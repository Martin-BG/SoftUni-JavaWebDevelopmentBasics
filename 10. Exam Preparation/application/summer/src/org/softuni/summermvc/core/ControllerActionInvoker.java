...
public class ControllerActionInvoker {
    ...
    private boolean isBindingResult(Parameter parameter) {  //NOTE - implemented BindingResult (errors) class
        if (parameter == null) {
            return false;
        }
        ...
        return
                parameterTypeName.equals(BindingResult.class.getSimpleName());
    }
    ...
    private boolean isBindingModel(Parameter parameter) {
        return parameter != null
                ...
                && !this.isBindingResult(parameter); //NOTE - implemented BindingResult (errors) class
    }
    ...
    private void populateBindingModel(Object bindingModel, HttpSoletRequest request) {
        try {
            BindingResult bindingResult = (BindingResult) this.dependencyContainer.getObject(BindingResult.class.getSimpleName());//NOTE - implemented BindingResult (errors) class
            ...
            Arrays.stream(bindingModel.getClass()
                    ...
                    .forEach(field -> {
                        ...
                        if (request.getBodyParameters().containsKey(field.getName())) {
                            ...
                            System.out.println("[" + bindingModel.getClass().getName() + "] [" + field + "] field not found in request body parameters!"); // NOTE: added report for missing field in populateBindingModel
                        }*/

                        try { //NOTE - implemented BindingResult (errors) class
                            if (field.get(bindingModel) == null) {
                                bindingResult.addError("Mapping of the \"" + field.getName() + "\" field failed.");
                            }
    ...
    private Object[] getActionArguments(Method action, Set<Object> parameters) throws InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        for (int i = 0; i < actionParameters.length; i++) {
            ...
            } else if (this.isBindingResult(currentParameter)) {//NOTE - implemented BindingResult (errors) class
                actionArguments[i] =
                        this.dependencyContainer.
                                getObject(BindingResult.class.getSimpleName());
